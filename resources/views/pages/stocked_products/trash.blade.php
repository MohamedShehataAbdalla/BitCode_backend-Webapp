...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">العمليات</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ المخزون المحذوفة</span>
						</div>
					</div>
				</div>
				<!-- breadcrumb -->
										...
										<tbody class="">
                                            @forelse ($stockedProducts as $stockedProduct)
                                                <tr>
                                                    ...
                                                    <td class="text-center">{{ $stockedProduct->deleted_at }}</td>
                                                    <td class="text-center">

                                                        {{-- @can('class-show') --}}
                                                            <a class="btn btn-success btn-sm" href="{{ route('stocked_products.restore',$stockedProduct->id) }}" title="أسترجاع"><i class="fas fa-trash-restore"></i></a>
                                                        {{-- @endcan --}}

                                                       {{-- @can('حذف قسم') --}}
                                                        <a class="btn btn-sm btn-danger modal-effect" data-effect="effect-sign" href="#delete_Modal" title="حذف"
                                                            data-id="{{ $stockedProduct->id }}" data-name="{{ $stockedProduct->product->name }}" data-toggle="modal" >
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content modal-content-demo">
                        <div class="modal-header">
                            <h6 class="modal-title">حذف المخزون</h6>
                            <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                        </div>
                        <form action="{{ route('stocked_products.delete') }}" method="post">
                            {{ method_field('delete') }}
                            {{ csrf_field() }}
