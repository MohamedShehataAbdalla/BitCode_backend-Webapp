											...
											<div class=" tab-menu-heading">
												<div class="tabs-menu1">
													...
													<ul class="nav panel-tabs main-nav-line">
														...
														<li class="nav-item"><a href="#tab3" class="nav-link" data-toggle="tab">حالات الدفع</a></li>
														<li class="nav-item"><a href="#tab4" class="nav-link" data-toggle="tab">المرفقات</a></li>
													</ul>
												</div>
											</div>
											<div class="panel-body tabs-menu-body main-content-body-right border-top-0 border">
												<div class="tab-content">
                                                            ...
                                                            <table class="table table-striped" >
                                                                <tbody>
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td>
                                                                            <span class="badge badge-pill {{ $invoice->payment_status == 'paid' ? 'badge-success' : ($invoice->payment_status == 'partial' ? 'badge-warning' : 'badge-danger') }}">
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td>{{ $invoice->employee->first_name }} {{ $invoice->employee->middle_name }} {{ $invoice->employee->last_name }}</td>
                                                                        <th scope="row">إجمالي الفاتورة</th>
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td>{{ ($invoice->vat_percentage * 100) }} %</td>
                                                                        <th scope="row">قيمة الضريبة</th>
                                                                        <td>{{ $invoice->vat_value }}</td>
                                                                        <th scope="row">تكلفة التوصيل</th>
                                                                        <td>{{ $invoice->delivery_cost }} </td>
                                                                        <th scope="row">صافي الفاتورة</th>
                                                    ...
                                                    <div class="tab-pane" id="tab2">
														<div class="table-responsive">
                                                            <table class="table text-md-nowrap table-hover table-striped" id="invoice_details">
                                                                <thead>
                                                                    <tr>
                                                                        <th class="wd-1p border-bottom-0">#</th>
                                                                        <th class="wd-20p border-bottom-0">القسم</th>
                                                                        <th class="wd-25p border-bottom-0">المنتج</th>
                                                                        <th class="wd-15p border-bottom-0 text-center">السعر</th>
                                                                        <th class="wd-15p border-bottom-0 text-center">الكمية</th>
                                                                        <th class="wd-15p border-bottom-0 text-center">الإجمالي</th>
                                                                ...
                                                                <tbody class="">
                                                                    @forelse ($invoice->invoice_details as $item)
                                                                        <tr>
                                                                            ...
                                                                            <td>
                                                                                <a class="" href="{{ route('products.show',$item->product->id) }}">{{ $item->product->name }}</a>
                                                                            </td>
                                                                            <td class="text-center">{{ $item->price }}</td>
                                                                            <td class="text-center">{{ $item->quantity }} {{ $item->product->unit->name }}</td>
                                                                            <td class="text-center">{{ $item->item_total }}</td>
                                                                            <td class="text-center {{ $item->status == 1 ? 'text-success' : 'text-danger' }}">
                                                                                {{ $item->status == 1 ?  'مفعل' : 'غير مفعل' }}
                                                                            </td>
                                                                        </tr>
                                                                    @empty
                                                                        <tr class="thead-light text-center">
                                                                            <td colspan="7">لا يوجد تفاصيل</td>
                                                                        </tr>
                                                                    @endforelse
                                                                </tbody>
                                                            </table>
													...
													<div class="tab-pane" id="tab4">
                                                                    ...
                                                                    <div class="row mb-3">
                                                                        <div class="col-lg-1 col-md-1">
                                                                            ...
                                                                            <div class="col-lg-6 col-md-6">
                                                                                <input type="text" name="name" id="name" class="form-control text-center" placeholder="صورة الفاتورة" value="{{ old('name') }}" >
                                                                                @error('name')
														...
														<div class="table-responsive">
                                                            <table class="table text-md-nowrap table-hover table-striped">
                                                                ...
                                                                <tbody>
                                                                    @forelse ($invoice->invoice_attachments as $attachment)
                                                                        <tr>
                                                                            <td>{{ $loop->iteration }}</td>
                                                                            ...
                                                                            <td>                                                                                      
                                                                                ...
                                                                                @if ($attachment->status == 0)
                                                                                    <a class="btn btn-outline-secondary btn-sm" href="{{ route('invoice_attachments.active',$attachment->id) }}" title="تفعيل"><i class="fas fa-lightbulb"></i></a>
                                                                                @else
                                                                                    <a class="btn btn-secondary btn-sm" href="{{ route('invoice_attachments.deactive',$attachment->id) }}" title="إلغاء التفعيل"><i class="far fa-lightbulb"></i></a>
                                                                                @endif
                                                                                ...
                                                                                <form action="{{ route('invoice_attachments.download') }}" method="POST" enctype="multipart/form-data" class="d-inline">
                                                                                    @csrf()
                                                                                    @method('POST')
            ...
            <div class="modal fade" id="delete_Modal">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content modal-content-demo">
                        ...
                        <form action="{{ route('invoice_attachments.delete') }}" method="post">
                            {{ method_field('delete') }}
                            {{ csrf_field() }}
